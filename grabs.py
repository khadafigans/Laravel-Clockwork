...
def should_exclude_subdomain(hostname):
    ...
    excluded_subdomains = ['cpanel.', 'mail.', 'whm.', 'www.']
    
    for excluded in excluded_subdomains:
        if hostname_lower.startswith(excluded):
            return True
...
def extract_root_domain(hostname):
    ...
    NOW RETURNS SUBDOMAIN TOO (except cpanel/mail/whm/www)
    """
    if not hostname:
        return None
    ...
    # Check if subdomain should be excluded
    if should_exclude_subdomain(hostname):
        return None
    ...
    if len(parts) >= 3:
        ...
        if potential_tld in special_tlds:
            # For special TLDs, check root domain validity
            root = '.'.join(parts[-3:])
            if is_staging_or_cloud_domain(root) or not is_valid_root_domain(root):
                return None
            # Return full hostname (with subdomain)
            return hostname
    
    # Standard TLD (e.g., .com, .net, .org)
    # Return full hostname (including subdomain) if valid
    if len(parts) >= 2:
        # Check if hostname is valid (not hosting/staging)
        if not is_valid_root_domain('.'.join(parts[-2:])):
            return None
        # Return full hostname (with subdomain)
        return hostname
...
LARAVEL_QUERIES = [
    # Laravel framework identifiers
    'http.html:"laravel_session"',
    'http.html:"XSRF-TOKEN"',
    'http.html:"Laravel"',
    'http.html:"Illuminate\\\\"',
    
    # Laravel error pages
    'http.html:"Laravel" "Whoops"',
    'http.html:"Illuminate\\\\Foundation"',
    'http.html:"vendor/laravel"',
    
    # Laravel headers
    'http.headers:"Set-Cookie: laravel_session"',
    'http.headers:"Set-Cookie: XSRF-TOKEN"',
    
    # Laravel with common ports
    '"Laravel" port:80',
    '"Laravel" port:443',
    'laravel_session port:80',
    'laravel_session port:443',
    
    # Laravel debug mode
    'http.html:"APP_DEBUG" "true"',
    'http.html:"APP_ENV" "local"',
    'http.html:"laravel_session" "clockwork"',
    'http.html:"XSRF-TOKEN" "clockwork"',
    
    # Laravel debug indicators (Clockwork often present)
    'http.html:"debugbar" "laravel"',
    'http.html:"laravel" "debug"',
    
    # Laravel session indicators
    'http.html:"laravel_session" port:80',
    'http.html:"laravel_session" port:443',
    'http.html:"XSRF-TOKEN" "laravel"',
    
    # Laravel framework fingerprints
    'http.html:"laravel/framework"',
    'http.html:"illuminate/support"',
    '"Laravel" "framework" port:80',
    '"Laravel" "framework" port:443',
    
    # JSON responses typical for Clockwork
    'http.html:"\\"version\\":" "clockwork"',
    'http.html:"\\"id\\":" "clockwork"',
    
    # Development environments (more likely to expose Clockwork)
    '"Laravel" "dev" port:80',
    '"Laravel" "staging" port:443',
        ...
        while attempt < 10:
            ...
            try:
                ...
                with lock:
                    ...
                    print(f"\n{Fore.MAGENTA}Query: {query}{Style.RESET_ALL}")
                    print(f"{Fore.MAGENTA}Page {page}: {len(matches)} matches (Total available: {total_results}){Style.RESET_ALL}")
                        ...
                        if hostnames:
                            for hostname in hostnames:
                                if not is_ip(hostname):
                                    # Validate hostname (filters staging/cloud/excluded subdomains)
                                    validated_hostname = extract_root_domain(hostname)
                                    
                                    if not validated_hostname:
                                        # Filtered out - hosting/staging/cloud/excluded subdomain
                                        print(f"{Fore.RED}✗ BLOCKED: {hostname}{Style.RESET_ALL}")
                                        continue
                                    
                                    if validated_hostname not in result_set:
                                        result_set.add(validated_hostname)
                                        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                                        print(f"{Fore.GREEN}✓ SAVED: {validated_hostname}{Style.RESET_ALL}")
                                        with open(host_output_path, "a", encoding='utf-8') as f:
                                            f.write(validated_hostname + "\n")
                                    else:
                                        print(f"{Fore.CYAN}• DUPLICATE: {validated_hostname} (already saved){Style.RESET_ALL}")
                ...
                print(f"\n{Fore.RED}[ERROR] Attempt {attempt+1}/10 failed: {str(e)[:100]}{Style.RESET_ALL}")
                attempt += 1
                if "API key" in str(e) or "401" in str(e):
                    print(f"{Fore.RED}[!] API key appears invalid! Check SHODAN_API_KEY in grabs.py{Style.RESET_ALL}")
                    break
                time.sleep(2)
...
def grab_domains():
    ...
    try:
        for idx, country in enumerate(country_list):
            ...
            print(f"{Fore.LIGHTGREEN_EX}Starting search for Laravel sites in {country if country else 'ALL'} (quota: {this_country_quota})...{Style.RESET_ALL}")
            ...
            query_count = 0
            total_queries = len(LARAVEL_QUERIES) * len(date_ranges)
            ...
            for date_start, date_end in date_ranges:
                ...
                for query_base in LARAVEL_QUERIES:
                    ...
                    print(f"{Fore.YELLOW}[Query {query_count}/{total_queries}]{Style.RESET_ALL} {query}")
                    print(f"{Fore.CYAN}Pages: {len(page_numbers)} | Results: {len(result_set)}/{this_country_quota}{Style.RESET_ALL}")
